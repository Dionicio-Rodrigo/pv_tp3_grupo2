...
const DashBoard = () => {
  ...
  const totalMensajes = mensajes.reduce(
    (total, mensaje) => total + mensaje.cantidad,
    0,
  );
    ...
    <Grid container spacing={3} sx={{ p: 3 }}>
      ...
      <Grid xs={12} md={6}>
        ...
        <Paper>
          <List>
            <ListItem divider>
              <ListItemText primary='"Nutrición con Energía" se suspende el próximo viernes' />
            </ListItem>
            <ListItem divider>
              <ListItemText primary="La inscripción para el Taller de SQL ha terminado" />
            </ListItem>
            <ListItem divider>
              <ListItemText primary='"Probabilidad en Acción" prepara su presentación final' />
            </ListItem>
            <ListItem>
              <ListItemText primary="Se compraron nuevos filamentos para Impresiones 3D" />
            </ListItem>
          </List>
        </Paper>
      </Grid>
          ...
          <List>
            {mensajes.map((m, index) => (
              <ListItem key={index} divider={index !== mensajes.length - 1}>
                <ListItemText
                  primary={m.de}
                  secondary={`${m.cantidad} mensajes nuevos`}
                />
              </ListItem>
            ))}
          </List>
